...
import type { SearchRequestPayload, StructuredSearchPayload } from "@/lib/search-schema";
...
function buildSearchPreferences(query: string, parsedQuery: ParsedSearchQuery | null, hints: QueryHints, travelType: SearchType): SearchPreferences {
  return {
    travelType,
...
function buildTargetGoal(target: SearchTarget, query: string, parsedQuery: ParsedSearchQuery | null, hints: QueryHints) {
...
export async function POST(req: NextRequest) {
  ...
  const hints = {
    ...
    dateWindow: structured?.departDate?.trim() || structured?.returnDate?.trim() || baseHints.dateWindow,
  };
        ...
        await Promise.all(
          activeTargets.map(async (target) => {
            ...
            try {
              ...
              controller.enqueue(
                encoder.encode(encodeEvent({ type: "AGENT_DONE", platform: target.name, result: result.total > 0 ? result : null }))
              );
            } catch (error) {
              failedAgents += 1;
              controller.enqueue(
